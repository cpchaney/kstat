plot_interaction <- function(interaction_id, config, cell_types, point_size = 0.5, zoom_box = NULL) {
  ...
  ligand_signal <- getSignalCoordinates("ligand", ligand, config$scaled_height, config$scaled_width) %>%
    dplyr::filter(magnitude > ligand_threshold$threshold_value)

  receptor_signal <- getSignalCoordinates("receptor", receptor, config$scaled_height, config$scaled_width) %>%
    dplyr::filter(magnitude > receptor_threshold$threshold_value)
  ...
  p_base <- ggplot(signal_table, aes(X, Y)) +
    ...
    geom_point(aes(colour = channel, alpha = magnitude), size = point_size, stroke = 0) +
  ...
  if (!is.null(zoom_box)) {
    ...
    signal_zoom <- signal_table %>%
      dplyr::filter(between(X, zoom_x[1], zoom_x[2]), between(Y, zoom_y[2], zoom_y[1]))
